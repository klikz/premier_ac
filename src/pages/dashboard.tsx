...
export default function Dashboard() {
  ...
  function t3Table() {  return (    <div      style={{        width: "100%",        overflowX: "auto",        borderRadius: 20,        background: "white",      }}    >      <Table        style={{          width: "100%",        }}      >        {/* HEADER */}        <TableHeader>          {tableT3.getHeaderGroups().map((hg) => (            <TableRow              key={hg.id}              style={{                background:                  "linear-gradient(90deg, #1e3a8a 0%, #2563eb 100%)",              }}            >              {hg.headers.map((header) => (                <TableHead                  key={header.id}                  style={{                    width: `${100 / hg.headers.length}%`,                    fontSize: 26,                    color: "white",                    fontWeight: "bold",                    padding: "22px 18px",                    borderBottom: "2px solid #3b82f6",                    textAlign: "center",                    letterSpacing: 1,                  }}                >                  {flexRender(                    header.column.columnDef.header,                    header.getContext()                  )}                </TableHead>              ))}            </TableRow>          ))}        </TableHeader>        {/* BODY */}        <TableBody>          {tableT3.getRowModel().rows.map((row, index) => (            <TableRow              key={row.id}              style={{                backgroundColor:                  index % 2 === 0 ? "#f8fafc" : "#eef4ff",              }}            >              {row.getVisibleCells().map((cell) => (                <TableCell                  key={cell.id}                  style={{                    width: `${100 / row.getVisibleCells().length}%`,                    fontSize: 28,                    padding: "20px 18px",                    color: "#0f172a",                    fontWeight: 600,                    borderBottom: "1px solid #dbeafe",                    textAlign: "center",                  }}                >                  {flexRender(                    cell.column.columnDef.cell,                    cell.getContext()                  )}                </TableCell>              ))}            </TableRow>          ))}        </TableBody>      </Table>    </div>  )}// function t1Table() {
  //   return <Table style={{}}>
  ...
  return (
    <div
      ...
      <div style={{ display: "flex", gap: 20, marginBottom: 25, }}    >
        ...
        {/* DONE */}
        <div style={{ flex: 1, borderRadius: 20, padding: 30, background: "linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)", color: "white", boxShadow: "0 10px 25px rgba(37,99,235,0.25)", }}      >
          <div style={{ fontSize: 42, opacity: 0.9, }}        >
            Бажарилди
          </div>
          <div style={{ fontSize: 150, fontWeight: "bold", marginTop: 0, alignContent: 'center', display: 'flex', justifyContent: 'center' }}        >
      ...
      <div style={{ background: "white", borderRadius: 20, padding: 25, boxShadow: "0 8px 20px rgba(0,0,0,0.08)", }}    >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, }}      >
          ...
          <div style={{ background: "#eff6ff", color: "#2563eb", padding: "8px 16px", borderRadius: 12, fontWeight: "bold", }}        >
            {t3.length} модель
          </div>
        </div>
